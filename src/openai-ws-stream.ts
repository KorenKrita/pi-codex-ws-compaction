...
import { resolveWebSocketUrl, type RuntimeConfig } from "./config.ts";
import {
  ...
  return (model, context, options) => {
    ...
    queueMicrotask(() => {
      const run = async () => {
        ...
        const modelInfo = getModelDescriptor(model);
        if (!isConfiguredResponsesModel(model, cfg)) {
        ...
        // Pi's internal summarization calls do not carry a session id. They use full-input HTTP,
        // never HTTP previous_response_id. Normal configured-model turns require WebSocket.
        if (!sessionId) {
          return await fallbackToHttpResponses(model, context, options, eventStream);
        }
        if (!apiKey) {
          throw new Error("Configured Codex compaction models require an API key for WebSocket transport.");
        }
        if (transport === "sse") {
          throw new Error("Configured Codex compaction models require WebSocket transport.");
        }
        ...
        if (!session) {
          ...
          const url = managerOptions.url ?? resolveWebSocketUrl(model.baseUrl, cfg.websocketUrl);
          session = {
            manager: new OpenAIWebSocketManager({ ...managerOptions, url, maxRetries: 0, headers }),
        ...
        const prevResponseId = incrementalAllowed
          ? session.manager.previousResponseId ?? undefined
          : undefined;
          ...
          const unsubscribe = session.manager.onMessage((event: OpenAIWebSocketEvent) => {
            if (event.type === "response.completed") {
              ...
              setContinuationState(sessionId, {
                ...
                modelKey: currentModelKey,
                updatedAt: Date.now(),
                contextLength: capturedContextLength + 1,
              });
