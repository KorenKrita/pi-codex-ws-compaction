...
export function remoteCompactionV2EndpointUrl(
  ...
  return resolveHttpResponsesUrl(model.baseUrl);
}
...
export function buildRemoteCompactionHeaders(params: {
  ...
  const commonHeaders = withRemoteCompactionV2Feature(mergeHeadersCaseInsensitive(
    { authorization: `Bearer ${params.apiKey}` },
    codexIdentityHeaders,
    params.headers,
    { accept: "text/event-stream", "content-type": "application/json" },
  ));
  if (!isConfiguredResponsesModel(params.model, params.config)) {
...
export async function callRemoteCompactionEndpoint(params: {
  ...
  if (!isConfiguredResponsesModel(params.model, params.config)) {
    throw new Error("Remote compaction v2 is currently only enabled for configured Responses models.");
  }
...
export function buildCompactionSummaryText(model: Model<any>): string {
  const host = hostnameFromBaseUrl(model.baseUrl) ?? "configured Responses endpoint";
  return `Remote compaction applied for ${model.provider}/${model.id} via ${host}. Pi keeps this textual summary for portability while compatible future turns use the provider-native replacement history stored in compaction details.`;
}
